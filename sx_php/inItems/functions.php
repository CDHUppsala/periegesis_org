<?php
// Count Items - if more than 2 Items, Created Aside navigation
function sx_checkMultiItems($arr, $rows)
{
    $radioTemp = false;
    if (is_array($arr)) {
        $iTemp = 0;
...
function sx_getLinkPath($Table, $Link, $sTitle, $linkClass)
{
    ...
    if (strpos($Title, ".png") > 0 || strpos($Title, ".jpg") > 0 || strpos($Title, ".webp") > 0 || strpos($Title, ".gif") > 0 || strpos($Title, ".svg") > 0 || strpos($Title, ".jpeg") > 0) {
...
/**
 * Get the Button Links for Item Elements
 */
function sx_getButtons($sTable, $mLink_1, $Title_1, $mLink_2, $Title_2)
{
    ...
    if (!empty($mLink_1)) {
        $linkClass = "";
        if (defined('sx_ButtonSectionClass_1') && !empty(sx_ButtonSectionClass_1)) {
    ...
    if (!empty($mLink_2)) {
        $linkClass = "";
        if (defined('sx_ButtonSectionClass_2') && !empty(sx_ButtonSectionClass_2)) {
