...
if (is_array($aResults)) { ?>
	<section class="jqNavMainToBeCloned">
		...
		<nav class="nav_aside jqNavAsideToggleNext">
			<?php
			...
			for ($r = 0; $r < $iRows; $r++) {
				...
				if ($radioGroups) {
					...
					if (intval($iAboutCategoryID) > 0 && $iAboutCategoryID != $iLoopCat) {
						...
						echo "<li><div>" . $sCategoryName . "</div><ul>";
						$radioCat = True;
					}
				} ?>
					<li><a href="<?= $strNavPath ?>agid=<?= $iAboutGroupID ?>&aboutid=<?= $iAboutID ?>"><?= $sTitle ?></a></li>
				<?php
