...
function populate_Filters(map, map_Filters, map_Legends, map_Download, geojson, file_Name) {
    ...
    const submitFilter = map_Filters.querySelector('.submit-filters');
    const resetFilters = map_Filters.querySelector('.reset_filters');
    ...
    submitFilter.addEventListener('click', () => apply_Filters(map, map_Filters, keyMap, map_Legends, geojson));

    // Download buttons
    //const geoBtn = map_Filters.querySelector('.download-geojson');
    const geoBtn = map_Download.querySelector('.download-geojson');
    geoBtn.addEventListener('click', () => download_GeoJSON(map_Legends));

    //const csvBtn = map_Filters.querySelector('.download-csv');
    const csvBtn = map_Download.querySelector('.download-csv');
    csvBtn.addEventListener('click', () => download_CSV(map_Legends));

    function resetContainer(mapFilters) {
        mapFilters.querySelectorAll('input[type="text"]').forEach(i => i.value = '');
        mapFilters.querySelectorAll('select').forEach(s => s.selectedIndex = 0);
    }

    resetFilters.addEventListener('click', () => resetContainer(map_Filters));
...
function apply_Filters(map, map_Filters, keyMap, map_Legends, geoJSON) {
    ...
    const typFeaturedVal = map_Filters.querySelector(".featured_type_filter")?.value;
    const textVal = map_Filters.querySelector(".text-filter").value.trim();
    const freTextVal = textVal.toLowerCase();

    const has_AnyFilterValue = [bookVal, typeFirstVal, typFeaturedVal, regionVal, textVal].some(Boolean);
    ...
    const filtered = geoJSON.features.filter(f => {
        ...
        const typeFeaturedMatch =
            !typFeaturedVal ||
            types.includes(typFeaturedVal);
        ...
        if (freTextVal !== "") {
            const book_Match = passages.some(
                p => typeof p === "string" && p.toLowerCase().includes(freTextVal)
            );

            const type_Match = types.some(
                p => typeof p === "string" && p.toLowerCase().includes(freTextVal)
            );
            const feature_Mach = types.some(
                p => typeof p === "string" && p.toLowerCase().includes(freTextVal)
            );

            const region_Match = region.toLowerCase().includes(freTextVal);
            ...
            const other_Match = Object.values(props).some(val => {
                ...
                return trimmed.toLowerCase().includes(freTextVal);
            });

            textMatch = book_Match || type_Match || feature_Mach || region_Match || other_Match;
        }

        return bookMatch && typeMatch && typeFeaturedMatch && regionMatch && textMatch;
    });
    ...
    if (has_AnyFilterValue) {
        ...
        if (typFeaturedVal) search_Name += '_' + typFeaturedVal;
    ...
    handle_LayerPopupHover(map, map_Legends, geoLayer, map_Filters, search_Name, format, true, initial_FileName);
}
