...
function bind____PopupsToGroup(group) {

    group.eachLayer(layer => {

        // recurse into sub-groups
        if (layer instanceof L.LayerGroup) {
            bind_PopupsToGroup(layer);
            return;
        }

        if (!layer.feature || !layer.feature.properties) return;

        const popupContent = sanitize_PopupContent(
            layer.feature.properties
        );

        // ───── LINE VISUAL LAYER ─────
        // if this layer has a hit layer, bind popup THERE
        if (layer.__hitLayer) {

            if (!layer.__hitLayer.getPopup()) {
                layer.__hitLayer.bindPopup(popupContent);
            }
        ...
        // ───── HIT LAYER ITSELF ─────
        // (defensive: if discovered directly)
        if (layer.__isHitLayer) {
...
function createHitLayer_first(feature, targetLayer) {
    return L.geoJSON(feature, {
        ...
        onEachFeature: (_, hitLayer) => {
            hitLayer.on("mouseover", () => {
                targetLayer.fire("mouseover");
                targetLayer.bringToFront();
                hitLayer.bringToFront();
            });
            hitLayer.on("mouseout", () => targetLayer.fire("mouseout"));
            //hitLayer.on("click", e => targetLayer.fire("click", e));
            hitLayer.on("click", () => {
                hitLayer.openPopup();
            });
        }
...
function create________HitLayer(feature, targetLayer) {
    return L.geoJSON(feature, {
        style: {
            color: "#000",
            weight: 15,
        ...
        onEachFeature: (_, hitLayer) => {
            ...
            hitLayer.on("click", e =>
                targetLayer.fire("click", e)
            );
        }
    });
}

function createHit__________________Layer(feature, targetLayer) {
    const hitLayer = L.geoJSON(feature, {
        style: {
            color: "#000",
            weight: 15,
            opacity: 0.001,   // invisible but hittable
            fillOpacity: 0
        },
        interactive: true
    });
...
function create____Hit____Layer(feature, targetLayer) {

    const hitGroup = L.geoJSON(feature, {
        style: {
            color: "#000",
            weight: 15,
...
function createHitLayer(feature, targetLayer) {
...
// Replaces tha above function
function create____LineInteractionLayer(feature, visualLayer, styleSet) {
    ...
    // Hover → affects visual layer
    hitLayer.on("mouseover", () => {
        visualLayer.bringToFront();
        hitLayer.bringToFront();
        visualLayer.setStyle(styleSet.hover);
    });

    hitLayer.on("mouseout", () => {
        visualLayer.setStyle(styleSet.base);
    });
...
function createLineInteractionLayer(feature, visualLayer, styleSet) {
    ...
    // 🔑 LINK BOTH LAYERS
    visualLayer.__hitLayer = hitLayer;
    hitLayer.__isHitLayer = true;
