...
function sx_GetFolderGroups()
{
	...
	$sql = "SELECT 
		...
		WHERE fg.Hidden = False AND (fgg.Hidden = False OR fgg.Hidden IS NULL) " . sLoginAnd . " 
...
if (is_Array($aResults)) { ?>
	...
	<section class="jqNavMainToBeCloned">
		<h2 class="head"><span><?= $strFolderGalleryMenuTitle ?></span></h2>
			...
			<ul>
				...
				for ($iRow = 0; $iRow < $iRows; $iRow++) {
					...
					if (intval($intListGroupID) != intval($loop1)) {
						...
						$strBoxView = "none";
						$strClassView = "";
						if (intval($i_FolderGroupID) == intval($intListGroupID)) {
							$strBoxView = "block";
					...
					if (intval($intListCatID) > 0) {
						$strClassView = "";
						if (intval($i_FolderGalleryID) == intval($intListCatID)) {
							$strClassView = 'class="open"';
						} ?>
						<li><a <?= $strClassView ?> href="<?= $strGoToPageName ?>fgid=<?= $intListCatID ?>"><?= $strListCatName ?></a></li>
				<?php
					}
