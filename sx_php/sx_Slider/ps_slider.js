...
$sx(function () {
    if ($sx('.jq_sx_slider').length) {
        sx_slider = $sx('.jq_sx_slider');
        sx_get_slider(sx_slider)
    };
});

function sx_get_slider(sx_slider) {
    ...
    /*
        1. Set the H/W ratio for the image
            - Check first if there exist a defined H/W ratio (0.5, 0.36, 0.4, 0.5625, 0.6, 0.75, 1, 1.5)
    ...
    */
    ...
    sx_slider.find("li.thamp_list").click(function () {
        if (!$sx(this).hasClass('active')) {
            sx_cycleImages($sx(this).index("li.thamp_list"));
        }
    });
    ...
    function sx_cycleImages(pos) {
        ...
        if (sx_next.attr('data-title').length) {
            ...
            sx_slider.find('.description').stop(true, false).fadeOut(300, function () {
                $sx(this).html('<h1>' + sx_title + '</h1>');
                if (sx_next.attr('data-notes').length) {
                    $sx(this).stop().append('<p>' + sx_next.attr('data-notes') + '</p>');
                }
                if (sx_next.attr('data-datetime').length) {
                    $sx(this).find('p').stop().append('<span> ' + sx_next.attr('data-datetime') + '</span>');
                }
            }).fadeIn(300);
        }
