...
$aResults = sx_getFilmsNav();
if (is_array($aResults)) {
	$strNavPath = "films.php?";
	if (intval($iFilmSubCatID) > 0) {
		$iFilmGroupID = $intFilmGroupID;
		$iFilmCatID = $intFilmCatID;
	}  elseif (intval($iFilmCatID) > 0) {
		$iFilmGroupID = $intFilmGroupID;
	}  ?>
<section class="jqNavMainToBeCloned">
	<h2 class="head slide_up jqToggleNextRight"><span><?=$str_FilmMenuTitle ?></span></h2>
	...
	for ($iRow=0; $iRow < $iRows; $iRow++) {
		...
		if (intval($intMenuGroupID) != intval($loop1)) {
			...
			<li><a href="<?=$strNavPath?>filmGroupID=<?=$intMenuGroupID?>"><?=$strMenuGroupName?></a></li> <?php 
			} else {
				$levelUL1 = True;
				$strBoxView = "none";
				$strClassView = "";
				if (intval($iFilmGroupID) == intval($intMenuGroupID)) {
					$strBoxView = "block";
		...
		if ($levelUL1 && intval($intMenuCatID) != intval($loop2)) {
			...
			$levelUL2 = False;
				$strBoxView = "none";
				$strClassView = "";
				if (intval($iFilmCatID) == intval($intMenuCatID)) {
					$strBoxView = "block";
					$strClassView = ' class="open"';
				} 
			if (intval($intMenuSubCatID) == 0) { //The category has no subcategories ?>
					<li><a<?=$strClassView?> href="<?=$strNavPath?>filmCatID=<?=$intMenuCatID?>"><?=$strMenuCatName ?></a></li> <?php 
			} else {
				$levelUL2 = True ?>
		...
		if ($levelUL2) { 
			$strClassView = "";
			if (intval($iFilmSubCatID) == intval($intMenuSubCatID)) {
				$strClassView = ' class="open"';
			} ?>
							<li><a<?=$strClassView?> href="<?=$strNavPath?>filmSubCatID=<?=$intMenuSubCatID?>"><?=$strMenuSubCatName?></a></li> <?php 
		}
