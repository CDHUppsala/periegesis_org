...
if ($_SERVER["REQUEST_METHOD"] != "POST") {
    ...
    if (empty($strResetToken) || strlen($strResetToken) < 48 || strpos($strResetToken, " ") > 0) {
        header('Location: index.php?sx=spf');
        exit();
    } else {
        $_SESSION['ResetToken'] = $strResetToken;
    }
} elseif (isset($_SESSION['ResetToken'])) {

    $radioValidToken = true;
    if (empty($_POST['FormToken'])) {
        $radioValidToken = false;
        write_To_Log("Users Reset Password: Empty Token Hack-Attempt!");
    } elseif (!sx_valid_form_token("UserPasswordReset", $_POST["FormToken"])) {
        $radioValidToken = false;
        write_To_Log("Users Reset Password: Wrong Token Hack-Attempt!");
    }
    ...
    if ($radioContinue) {
        header('Location: ' . sx_ROOT_HOST_PATH . '?pg=message&change=yes');
        exit();
...
$strFormToken = sx_generate_form_token('UserPasswordReset', 128);
