<h2><%=lngByArea &": " & str_StatisticsYear%></h2>
<%
...
sql = "SELECT DISTINCTROW "&strGroupByField&", "_
	&" COUNT(OrderID) AS Count_Orders,  "_
	...
	&" sum(DiscountShipping) AS Sum_Shipp_Discount, "_
 	...
 	&" AVG(total) AS Avg_Total  "_
	&" FROM orders "_
	&" WHERE total > 0 "_
	& strYearWhere _
	&" GROUP BY "&strGroupByField&" "_
	&  strOrderBy
'Response.Write sql
...
if strExport = "" then %>
<div class="coloredBG">
<form style="padding-bottom: 5px; font-weight: bold" action="<%=Request.ServerVariables("PATH_INFO")%>?by=Area" name="GroupBy" method="post">
   <%=lngSelectGroupByField%>: <select name="GroupByField" size="1">
      <%if strGroupByField = "city" then sxSelected = " selected" else sxSelected = ""%>
      <option value="City"<%=sxSelected%>><%=lngCity%></option>
      <%if strGroupByField = "Country" then sxSelected = " selected" else sxSelected = ""%>
      <option value="Country"<%=sxSelected%>><%=lngCountry%></option>
      <%if strGroupByField = "State" then sxSelected = " selected" else sxSelected = ""%>
      <option value="State"<%=sxSelected%>><%=lngState%></option>
   </select>
...
for each x in rs.fields
	...
	if strExport = "" then  %>
			<th <%=strAlign%>>
				<a href="<%=Request.ServerVariables("PATH_INFO")%>?by=Area&orderBy=<%=strQuery %>"><span class="<%=sortColor%>"><%=s_Name%></span></a>
			</th>
	<%else %>
			<th <%=strAlign%>>
