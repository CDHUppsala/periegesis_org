...
$sx(window).on('load', function () {
    if ($sx("#jqThumpsBG li").length) {
        var $currpos = 0;
        var $elCount = $sx("#jqThumpsBG li").length - 1;
        $sx("#jqArrowPhotoNav").css("display", "block");
        $sx("#jqThumpsBG li").click(function () {
        ...
        if (int_PhotoID > 0) {
            ...
            $sx("#jqThumpsBG > li:eq(0)").click();
        }

        //Change image by arrows
        $sx("#jqArrowPhotoNav .jqNavArrows").click(function () {
            var $arrow = $sx(this);
            if ($arrow.attr("id") == "jqLeft") {
                if ($currpos == 0) {
                    $sx("#jqThumpsBG li:eq(" + $elCount + ")").click();
                } else {
                    $sx("#jqThumpsBG li:eq(" + ($currpos - 1) + ")").click();
                }
            } else {
                if ($currpos == $elCount) {
                    $sx("#jqThumpsBG li:eq(0)").click();
    ...
    if ($sx("#jqBigPhoto").length) {
        $sx("#jqBigPhoto").click(function () {
            ...
            var radioMobile = $sx(".sxNavMarker").is(":visible");
