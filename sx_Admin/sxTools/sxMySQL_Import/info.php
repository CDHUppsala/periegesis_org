...
$peakMemoryUsageBytes = number_format($peakMemoryUsageBytes, 0,'',' ');
...
<div class="maxWidth">
    ...
    <ul class="display_none">
        ...
        <li><b>Obs!</b> Since <b>JSON Files</b> are loaded in the server memory, their <b>size</b> must not exceed about <?php echo (int)(ini_get('memory_limit')) / 10; ?>M. Otherwise, you must splitt it into two or more files.</li>
        <li>Before importing the file, the program checks:
            <ul>
                <li>The uniqueness of all <b>Primary Keys</b>.</li>
                <li>The identity between the file's <b>Field Names</b> and the table's <b>Column Names</b>.</li>
    ...
    <ul class="display_none">
        <li>All <b>Primary Keys</b> (PK) in the file must have a unique value, usually but not necessarily an auto-incremented integer.</li>
    ...
    <ul class="display_none">
        <li>Only the records in the Import File that have the <b>same</b> Primary Key (PK) value with that of the Database Table will be updated.</li>
        <li>New records, with a PK value that not exist in the Database Table, will not be added.</li>
        <li>If you wish to update old records and add new ones, Update first and Insert thereafter.</li>
    </ul>
</div>