<script>
    var sx_create_csv_table = function($, constant_tableData) {
        ...
        function sx_renderRows() {
            ...
            $.each(slicedData, function(index, rowData) {
                ...
                $.each(columnNames, function(index, columnName) {
                    ...
                    // Default to an empty string if the value is null or undefined
                    // Convert numbers to strings
                    columnValue = columnValue == null ? '' : String(columnValue);

                    if (columnValue.length < 255) {
                        row += '<td class="' + columnName + '">' + columnValue + '</td>';
                    } else {
                        row += '<td>' + columnValue.slice(0, 200) + ' [...]</td>';
                    }
