...
/*
echo '<pre>';
print_r($arrUpdateableFields);
echo '</pre>';
*/

$arrUpdateableFields = [
    'ProductPrice' => 13,
    'ProductGrossPrice' => 13,
    'OfferRate' => 13,
    'OfferPrice' => 13,
    'ShowAsOffer' => 9,
    'OfferStartDate' => 'DATE',
...
/**
 * Add to field names the field Quantity forn the table shop_order_items
 * Keep the original variable $strSelectedFields (with fields from product table) to get related tables
 * Redifine $strSelectedFields to $str_SelectedFields for the rest of this page and write its values manually
 */

$strSelectedFields = "ProductID, GroupID, CategoryID, SubCategoryID, SubSubCategoryID";
$str_SelectedFields = "p.ProductID, p.GroupID, p.CategoryID, p.SubCategoryID, p.SubSubCategoryID, p.SexGroup, p.ProductName, COUNT(o.Quantity) AS Sale_Total, p.ProductImages, p.ProductPrice, p.ProductGrossPrice, p.OfferRate, p.OfferPrice, p.ShowAsOffer, p.OfferStartDate, p.OfferEndDate, p.ShowAsNew, p.ShowAsSelected, p.ShowInFirstPage";
...
if (!empty($_GET["RequestTable"])) {
    ...
    $sql = "SELECT $str_SelectedFields FROM $request_Table AS p LEFT JOIN shop_order_items AS o ON p.ProductID = o.ProductID GROUP BY p.ProductID,  o.ProductID LIMIT 1";
...
if ($radio_TablesWithLoginAdminID && intval($intLoginUserLevel) > 1) {
    ...
    $strFirstWhere = "p.{$strPK}  > 0 ";
}

$strSQL = "SELECT " . $str_SelectedFields . " FROM " . $request_Table . " AS p LEFT JOIN shop_order_items AS o ON p.ProductID = o.ProductID
...
$strSQL .= " GROUP BY p.ProductID,  o.ProductID";
 ...
 */
$intRecordCount = sx_getJoinRecordCount($strSQL);
...
-->
    ...
    <div id="navBG">
         ...
         * @$str_SelectedFields: visible fields for the current table
                                        ...
                                        <?php
                                            ...
                                            <?php
                                                ...
                                                if (@$arrUpdateableFields[$xName] == 13 && $_SESSION["UpdateMode"]) {
                                                    $strRName = $xName . "[]"; ?>
                                                <td>
                                                    <input maxlength="4" title="<?= lngMaxNumber ?>: 9999" style="text-align: right" size="4" type="text" value="<?= $xValue ?>" name="<?= $strRName ?>" onChange="validateZeroNumber(this)">
                                                </td>
                                            <?php
                                                } else {
                                            ?>
                                            ...
                                            <?php
                                                ...
                                                if (@$arrUpdateableFields[$xName] == 13 && $_SESSION["UpdateMode"]) {
                                                    $strRName = $xName . "[]"; ?>
                                                <td>
                                                    <input style="text-align: right" size="4" type="text" value="<?= $xValue ?>" name="<?= $strRName ?>" onChange="IsAllNumeric(this)">
                                                </td>
                                            <?php
                                                } else {
                                            ?>
                                            ...
                                            <?php
                                                ...
                                                if (@$arrUpdateableFields[$xName] == "TIME" && $_SESSION["UpdateMode"]) {
                                                    $strRName = $xName . "[]"; ?>
                                                <td>
                                                    <input type="time" value="<?= $xValue ?>" name="<?= $strRName ?>">
                                                </td>
                                            <?php
                                                } else {
                                            ?>
                                            ...
                                            <?php
                                                ...
                                                <td>
                                                    <div>
                                                        ...
                                                        <?php } ?>
                                                        <input type="date" value="<?= $xValue ?>" name="<?= $strRName ?>">
                                                    </div>
                                                </td>
                                            <?php
                                                } else { ?>
                                            ...
                                            <?php
                                                ...
                                                if ($xType == "STRING" || $xType == "VAR_STRING") {
                                                    $strLinks = "";
                                                    if (!empty($xValue)) {
                                                        $strLinks = sx_getLinksAndImages($orderByFieldName, $xName, $xValue, true);
                                                    } ?>
                                                <td><?= $strLinks ?></td>
                                            <?php
