...
$boolGetRecords = false;
$strGetReordsWhere = '';
$arr_EditResults = '';

/**
 * Get the Field Names and Field Types for adding and editing records
 * Get also records FOR editing - but not FOR editing/updating
...
$sql = "SELECT " . $strTopRecords_1 . " * FROM " . $request_Table . $strGetReordsWhere . " " . $strLimitRecords_1;

$stmt = $conn->query($sql);
$rs = $stmt->fetch(PDO::FETCH_BOTH);
if ($boolGetRecords && $rs) {
    $arr_EditResults = $rs;
}
...
{
    $arrAddFields = null;
    $arrAddPrepareValues = null;
    $arrAddValues = null;
    $strUppdatePrepare = null;
    $arrUppdateValues = null;
    $boolUppdate = false;
    if ($action == "update") {
        $boolUppdate = true;
    }
    ...
    for ($i = 0; $i < $iCount; $i++) {
        ...
        if (isset($_POST["Add" . $xName]) && !empty($_POST["Add" . $xName])) {
            ...
            */
            ...
            if ($strRelatedTableName != "" && $strRelatedFieldName != "") {
                $conn = dbconn();
                /**
                ...
                if ((int) $tempID > 0) {
                    ...
                    $xValue = $conn->lastInsertId(); //The ID-Number of the added record
                    $fstmt = null;
                }
            }
        ...
        if ($i > 0) {
            if ($xType == 'LONG' || $xType == 'LONGLONG') {
                ...
                if (empty($xValue) || (!sx_IsDate($xValue) && !sx_IsDateTime($xValue))) {
                    $xValue = null;
                }
