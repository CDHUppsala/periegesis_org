...
// starts from 60% of lightening of current color and ends with 0% = current colot
...
// starts with 10% and ends with 40% darkening of current color
...
function ps_get_palletes(color) {
    ...
    //Lighten current color by 10% in X steps
    ...
    //Darken current color by 10% in X steps
    ...
    for (x = 0; x < 2; x++) {
        clr = tinycolor(array[x]);
        light_g = sx_get_light_gradients(clr, 10, 6, five_gray)
        dark_g = sx_get_dark_gradients(clr, 10, 3)
    ...
    root_variables.append("--basic-color: " + basic_color + ";\n\n");
    str_rgb_strings += "--basic_color: " +
        str_replace_rgb(tinycolor(basic_color).toRgbString()) + ";\n\n";
    for (r = 0; r < arr_basic_tints.length; r++) {
        sufix = (r+1) * 10;
        root_variables.append("--lighten-" + sufix + ": " + arr_basic_tints[r] + ";\n");
        str_rgb_strings += "--lighten_" + sufix + ": " +
            str_replace_rgb(tinycolor(arr_basic_tints[r]).toRgbString()) + ";\n";
    }

    //Darken - shadows of black by 10%
    var arr_basic_shades = root_basic_shades.split(',');
    root_variables.append("\n");
    str_rgb_strings += "\n";
    for (r = 0; r < arr_basic_shades.length; r++) {
        sufix = (r+1) * 10;
        root_variables.append("--darken-" + sufix + ": " + arr_basic_shades[r] + ";\n");
        str_rgb_strings += "--darken_" + sufix + ": " +
            str_replace_rgb(tinycolor(arr_basic_shades[r]).toRgbString()) + ";\n";
    }

    //Desaturate - shadowss of of gray by 10% (desaturation)
    var arr_desaturate = root_desaturate.split(',');
    root_variables.append("\n");
    str_rgb_strings += "\n";
    for (r = 0; r < arr_desaturate.length; r++) {
        sufix = (r+1) * 10;
        root_variables.append("--desaturate-" + sufix + ": " + arr_desaturate[r] + ";\n");
        str_rgb_strings += "--desaturate_" + sufix + ": " +
            str_replace_rgb(tinycolor(arr_desaturate[r]).toRgbString()) + ";\n";
    }
    ...
    // Append Brightness to root colors
    var arr_brighten = root_brighten.split(',');
    root_variables.append("\n");
    str_rgb_strings += "\n";
    for (r = 0; r < arr_brighten.length; r++) {
        sufix = (r+1) * 10;
        root_variables.append("--brighten-" + sufix + ": " + arr_brighten[r] + ";\n");
        str_rgb_strings += "--brighten_" + sufix + ": " +
            str_replace_rgb(tinycolor(arr_brighten[r]).toRgbString()) + ";\n";
    }
    ...
    for (r = length; r > 0; r--) {
        root_variables.append("--monochrom-" + r + ": " + arr_mono[r] + ";\n");
        str_rgb_strings += "--monochrom_" + r + ": " +
            str_replace_rgb(tinycolor(arr_mono[r]).toRgbString()) + ";\n";
    }
    ...
    for (r = 1; r < arr_analog.length; r++) {
        ...
        if (r != 3) {
            ...
            str_rgb_strings += "--analog_" + suffix + ": " +
                str_replace_rgb(tinycolor(arr_analog[r]).toRgbString()) + ";\n";
        }
    }
    ...
    for (c = 0; c < 2; c++) {
        ...
        if (c == 0) {
            ...
            str_rgb_strings += "\n--triad_left: " +
                str_replace_rgb(tinycolor(arr_c[steps_light]).toRgbString()) + ";\n";
            prefix = 'left';
        }
        if (c == 1) {
        ...
        for (r = 0; r < arrLength; r++) {
            ...
            str_rgb_strings += "--triad_" + prefix + '_' + (arr_length * 10) + ": " +
                str_replace_rgb(tinycolor(arr_c[r]).toRgbString()) + ";\n";
            arr_length--;
        }
    }
    ...
    for (c = 0; c < 3; c++) {
        ...
        if (c == 2) {
            ...
            str_rgb_strings += "\n--tetrad_right: " +
                str_replace_rgb(tinycolor(arr_c[steps_light]).toRgbString()) + ";\n";
            prefix = '--tetrad-right';
            prefix_rgb = '--tetrad_right';
        }
        var arrLength = arr_length;
        for (r = 0; r < arrLength; r++) {
            root_variables.append(prefix + '-' + (arr_length * 10) + ": " + arr_c[r] + ";\n");
            str_rgb_strings += prefix_rgb + '_' + (arr_length * 10) + ": " +
                str_replace_rgb(tinycolor(arr_c[r]).toRgbString()) + ";\n";
            arr_length--;
        }
    }
    ...
    for (c = 0; c < 2; c++) {
        ...
        for (r = 0; r < arrLength; r++) {
            ...
            str_rgb_strings += "--split_" + prefix + '_' + (arr_length * 10) + ": " +
                str_replace_rgb(tinycolor(arr_c[r]).toRgbString()) + ";\n";
            arr_length--;
        }
    }
    ...
    root_variables.append("\n--gray: " + arr_c[6] + ";\n");
    str_rgb_strings += "\n--gray_00: " +
        str_replace_rgb(tinycolor(arr_c[6]).toRgbString()) + ";\n";
    for (r = 0; r < arr_c.length; r++) {
        sufix = arr_length * 10;
        root_variables.append("--gray-" + sufix + ": " + arr_c[r] + ";\n");
        str_rgb_strings += "--gray_" + sufix + ": " +
            str_replace_rgb(tinycolor(arr_c[r]).toRgbString()) + ";\n";
        arr_length--;
    }
    ...
    /**
     * ============================================
     * Get all colors as pure RGB string
     * to be used as variables in rgba(var(--color), 0.X) colors
     * ============================================
     */

    root_variables.append("\/*\n To be used as variables in rgba(var(--color), 0.X)\n*\/\n\n");

    root_variables.append(str_rgb_strings);
