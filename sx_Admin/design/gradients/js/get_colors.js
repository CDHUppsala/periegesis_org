...
jQuery(function ($) {
    ...
    var basic_variants = [
        ...
        "--darken-shade", "--aturate-shade", "--desaturate-shade",
    ...
    var array_palette = ['--complement', '--triad', '--tetrad', '--split','--grey']

    for (var key in root_variables) {
        var i_pos = 100;
        var pos = key.indexOf('-', 3);
        if (pos > -1) {
            i_pos = key.lastIndexOf("-");
        }
        ...
        if (key.indexOf("--basic-color") > -1) {
            ...
            || key.indexOf("--saturate") > -1
            || key.indexOf("--desaturate") > -1) {
            curr_type = 'Basic color Scales by 10%';
        ...
        if (last_type != curr_type) {
            group_key = curr_key;
            if (last_type != "") {
                $root_flex.appendTo($color_wrapper);
                $color_wrapper.appendTo(insertRootColor);
            }
            insertRootColor.append('<h3' + title_message + '>' + curr_type + '</h3>');
            $color_wrapper = $('<div>').attr('class', 'root_color_wrapper').appendTo(insertRootColor);
            $root_flex = $('<div>').attr('class', 'root_flex').attr('id', 'group' + group_key).appendTo($color_wrapper);
            console.log(curr_key + ' ' + group_key);

        } else if (last_key != curr_key && basic_variants.includes(curr_key) === false) {
            $root_flex.appendTo($color_wrapper);
            $root_flex = $('<div>').attr('class', 'root_flex').appendTo($color_wrapper);
        }
        ...
        if (curr_key === group_key) {
            $('#group' + group_key).append(append_string);
            if (array_palette.includes(group_key)) {
                $('#group--basic').append(append_string);
            }
        } else {
            $root_flex.append(append_string)
        }
    ...
    if (radio_modal_overlay === false && radio_ajax_loaded_page) {
