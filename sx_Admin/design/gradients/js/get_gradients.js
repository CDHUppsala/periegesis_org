 ...
 */
    ...
    function get_cleared_title(str) {
        ...
        var title = '';
        if (arrTitle[0] == 'linear') {
            title += 'Linear: ';
        } else if (arrTitle[0] == 'ellipse') {
            title += 'Ellipse: ';
        } else if (arrTitle[0] == 'circle') {
            title += 'Circle: ';
        }
        title += arrTitle[1];
    ...
    var load_gradients = function () {
        ...
        var arr_prefixes = ['circle-', 'linear-', 'ellipse-'];
        ...
        */
        ...
        /**
         * FILTER
         * Create a new array object with keys containing substring(s) to filter classes
         */
         ...
         * replace tints by color variables (HEX colors), with or without opacity:
         *  -  (--tint-0 by --tint-20)
        ...
        */
        ...
        // console.log(Object.keys(classesByValues).length);
        ...
        $.each(classesByValues, function (className, backgroundImage) {
            ...
            $('<div>').attr('id',className).addClass('gradient_img').css({ 'background-color': bg_color, 'background-image': backgroundImage }).appendTo(imgDiv);

            saveDiv = $('<div>').addClass('html_to_img_wrap').appendTo(imgDiv);
            $('<label>').html('<button title="Save DIV content as PNG Image using File System" onclick="html_to_png_save(\''+ className +'\',\''+ className+'\');">Save as PNG</button>').appendTo(saveDiv);
            $('<label>').html('<button title="Download DIV content as PNG Image" onclick="html_to_png_download(\''+ className +'\',\''+ className+'\');">Download PNG</button>').appendTo(saveDiv);
            $('<label>').html('<button title="Download DIV content as SVG Image" onclick="html_to_svg_download(\''+ className +'\',\''+ className+'\');">Download SVG</button>').appendTo(saveDiv);
    ...
    var load_toggle_functions = function () {
        $('.jq_toggle_next').on('click', function () {
            ...
            var wrapper_to_fiexed_hight = $("#jq_gradient_inputs_wrapper").height();
            $sxThis.toggleClass('selected').parent().next('.gradients_section').slideToggle(300, function () {
                if ($sxThis.hasClass('selected')) {
                    ...
                    $body_scroll_animation.animate({
                        scrollTop: ((thisTop - wrapper_to_fiexed_hight - $sxHeaderHeight) + (-1 * parent_top))
                    }, 300);
                }
            });
        })
        ...
        $('.jq_toggle_rest').on('click', function () {
            // Check which verssion is used (separate or loaded by ajax)
            if ($body_scroll_animation === null || $body_scroll_animation.is('html') === false) {
                $body_scroll_animation = $body_highest_element.parent();
            }
            ...
            let wrapper_to_fiexed_hight = $("#jq_gradient_inputs_wrapper").height();
            $sxThis.parent('#jq_InsertGradients').find('h3').removeClass('selected');
            $sxThis
                .find('h3').addClass('selected')
                .end()
                .next()
                .siblings('.gradients_section').slideUp(200)
                .end().slideDown(200, function () {
