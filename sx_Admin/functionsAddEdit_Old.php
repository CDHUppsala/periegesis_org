...
function sx_getInsertUpdateRecords($action)
{
    ...
    if (is_array(ARR_FieldNames) && !empty(ARR_FieldNames)) {
        ...
        for ($i = 0; $i < $iCount; $i++) {
            ...
            } elseif (!empty($_POST["Distinct" . $xName])) {
            ...
            if ($i > 0) {
                if ($xType == 'LONG' || $xType == 'LONGLONG') {
                    ...
                    // Hash Admin Password: Add or Edit/Update
                    // =======================================
                    if (REQUEST_Table === 'admin_login') {
                        if ($xName === 'UserPassword' && !empty($xValue)) {
                            $str_UserPassword = $xValue;
                            $xValue = null;
                        }
                        if ($xName === 'UserPasswordHashed' && !empty($str_UserPassword)) {
                            $xValue = password_hash($str_UserPassword, PASSWORD_DEFAULT);
                        }
                    }
                } elseif ($xType == 'BLOB') {
                    if (!empty($xValue)) {
        ...
        if ($isUpdate) {
            return [$strUppdatePrepare, $arrUppdateValues];
        } else {
            return [$arrAddFields, $arrAddPrepareValues, $arrAddValues];
        }
    }
}
