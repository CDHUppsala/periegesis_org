...
function IsAllNumeric(field) {
    ...
    for (i = 0; i < sx.length; i++) {
        ...
        if (i == 1 && intNeg == 1 && intDec == 1) {
            first = true
        }
        if ((ValidChars.indexOf(Char) == -1 || intDec > 1 || intNeg > 1 || first == true) && repeat == true) {
            repeat = false
    ...
    //Comment to allow empty strings
    if (sx.length == 0) {
        alert("You must write a positive/negative integer, a decimal number of any length or 0!");
        field.focus();
        field.select();
    }
}
