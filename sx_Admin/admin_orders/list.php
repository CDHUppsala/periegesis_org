...
include dirname(__DIR__) . "/list_configFields.php";
...
include dirname(__DIR__) . "/list_multipleUpdates.php";
...
$strSelectedFields = "OrderID, CAST(OrderDate AS DATE) AS OrderDate, CustomerID, DeliveryAddressID, CONCAT(IF(CompanyName <> '' AND CompanyName IS NOT NULL,CONCAT(CompanyName,'<br>'),''), LastName, ' ', FirstName,'<br>', City, '<br>', Phone) AS Customer, Email, ShipDate, ShipPromiseDate, Total, PaidAmount, SuccessfulPayment, InProcess, Shipped, Completed, Cancelled";
...
if (isset($_GET["searchFieldName"]) && isset($_GET["searchFieldValue"])) {
    ...
    if (sx_checkTableAndFieldNames($tempName)) {
        if (in_array($tempName, $arrDatetimeField)) {
            $searchFieldNameWhere = " AND DATE({$tempName}) = $tempValue";
        } else {
...
include dirname(__DIR__) . "/list_searchFromContent.php";
...
if (!isset($_SESSION["PrimaryKeyName"]) || empty($_SESSION["PrimaryKeyName"])) {
    header("Location: main.php?clear=yes&msg=No+PK+Orders");
    exit();
} else {
...
include dirname(__DIR__) . "/list_navFunctions.php";
...
-->
    ...
    <div id="navBG">
        ...
        include dirname(__DIR__) . "/list_searchRelatedFields.php";
                    ...
                    ?>
                                    <tr>
                                        <td data-name="<?php echo $xName ?>">
                                            ...
                                            <a title="<?= lngEditRecord ?>" href="index.php?oid=<?= $intPK ?>">
                                    ...
                                    <?php
                                        ...
                                        if ($i == 0) { ?>
                                        <td data-name="<?php echo $xName ?>"><?= $xValue ?></td>
                                        <?php
                                        } else {
                                            if ($xType == "LONG") { // 3 Makes searchable the subcategories that are related to long intigers
                                                ...
                                                if ($xName === 'CustomerID') { ?>
                                                <td data-name="<?php echo $xName ?>"><a title="Vie Orders from this Customer" href="index.php?cid=<?= $xValue ?>"><?= sx_getRelatedFieldNameForList($xName, $xValue) ?></td>
                                            <?php
                                                } else { ?>
                                                <td data-name="<?php echo $xName ?>"><a title="<?= lngShowRecordsFromThisCategoryOnly ?>" href="list.php?searchFieldName=<?= $xName ?>&searchFieldValue=<?= $xValue ?>"><?= sx_getRelatedFieldNameForList($xName, $xValue) ?></td>
                                            <?php
                                                }
                                                ...
                                                if (@$arrUpdateableFields[$xName] == 13 && $_SESSION["UpdateMode"]) {
                                                    ...
                                                    $idName = "{$xName}_{$r}"; ?>
                                                <td data-name="<?php echo $xName ?>">
                                                    <input maxlength="4" title="<?= lngMaxNumber ?>: 9999" style="text-align: right" size="4" type="text" value="<?= $xValue ?>" id="<?= $idName ?>" name="<?= $strRName ?>" onChange="validateZeroNumber(this)">
                                                </td>
                                            <?php
                                                } else {
                                            ?>
                                                <td data-name="<?php echo $xName ?>"><?= sx_getRelatedFieldNameForList($xName, $xValue) ?></td>
                                            <?php
                                                }
                                                ...
                                                if (@$arrUpdateableFields[$xName] == 13 && $_SESSION["UpdateMode"]) {
                                                    ...
                                                    $idName = "{$xName}_{$r}"; ?>
                                                <td data-name="<?php echo $xName ?>">
                                                    <input style="text-align: right" size="4" type="text" value="<?= $xValue ?>" id="<?= $idName ?>" name="<?= $strRName ?>" onChange="IsAllNumeric(this)">
                                                </td>
                                            <?php
                                                } else {
                                            ?>
                                                <td data-name="<?php echo $xName ?>"><?= number_format($xValue, 2) ?></td>
                                            <?php
                                                }
                                            ...
                                            <?php
                                                ...
                                                <td data-name="<?php echo $xName ?>"><a title="Show Records from this Date Only" href="list.php?searchFieldName=<?= $xName ?>&searchFieldValue=<?= $xValue ?>"><?= $xValue ?></a></td>
                                            <?php
                                                }
                                                ...
                                                <td data-name="<?php echo $xName ?>"><?= $strLinks ?></td>
                                            <?php
                                                    ...
                                                    $strRName = $xName . "[]"; ?>
                                                <td data-name="<?php echo $xName ?>" <?= $changeColor ?>>
    ...
    <?php
    include dirname(__DIR__) . "/sxHelpFiles/helpSearch.php";
    include dirname(__DIR__) . "/errorMsgForClient.php";
    ?>
    ...
    <script>
        jQuery(function($) {
            $("input[id^='PaidAmount_']").each(function() {
                ...
                $(this).parent().append(" <span>" + (parseFloat(totalField.val()) - (parseFloat($(this).val()) || 0) + "</span>"));
            });
